...
class AnnotationScene(QtWidgets.QGraphicsScene):
    ...
    def load_image(self, image_path: str, image_data=None):
        self.clear()

        t0 = time.time()

        self.image_data = np.array(Image.open(image_path) if image_data is None else image_data)
        ...
        h, w, c = np.shape(self.image_data)
        self.image_item.setPixmap(QtGui.QPixmap(
            QtGui.QImage(self.image_data, w, h, w * c, QtGui.QImage.Format_RGB888)
        ))
...
class AnnotationView(QtWidgets.QGraphicsView):
    ...
    def zoomfit(self):
        ...
        self.fitInView(0, 0, w, h, QtCore.Qt.AspectRatioMode.KeepAspectRatio)
    ...
    def zoom(self, factor, point=None):
        ...
        if pix_widget > 100 or pix_widget < 0.01:
            return
