...
### 2.2 半精度推理
可以选择使用半精度推理（默认打开），大幅减少显存消耗（最大的模型以及MobileSAM不知为何，使用半精度效果奇差，本工具会强制使用单精度，但另外两个模型的半精度推理能够正常使用）
...
```yaml
force_model_type: l   # 或 b 或 mobile
```
再打开本工具即可
...
因此可以通过以下连招：
```
1. Q → 鼠标点点点 → E → E → Q → 鼠标点点点 → E → E → Q → ...      # 不带group id标注目标
2. Q → 鼠标点点点 → E → Q → Q → 鼠标点点点 → E → Q → Q → ...      # 带group id标注同一目标
3. Q → 鼠标点点点 → E → W → Q → 鼠标点点点 → E → W → Q → ...      # 带group id标注不同目标
```
进行快速标注。

同时鼠标点击选中一个目标的掩码msak后，可以按Tab键快速隐藏其他掩码（如果鼠标没有选择就全部隐藏了），防止其他掩码干扰视线。再次按下Tab键恢复。
